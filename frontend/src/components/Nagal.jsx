...
const Nagal = () => {
  ...
  const handleAddNew = () => {
    ...
    setFormData({ date: "", nagalNo: "", vendor: "", customerName: "", client: "", office: "", nagar: "", surveyNo: "", amount: "", commission: "", others: "", totalFee: "", received: "", balance: "" });
    setShowModal(true);
  };
  const handleEdit = (nagal, e) => {
    e.stopPropagation(); setIsEdit(true); setCurrentNagal(nagal);
    setFormData({ date: nagal.date, nagalNo: nagal.nagalNo, vendor: nagal.vendor, customerName: nagal.customerName, client: nagal.client?._id || nagal.client || "", office: nagal.office, nagar: nagal.nagar, surveyNo: nagal.surveyNo, amount: nagal.amount.toString(), commission: nagal.commission.toString(), others: nagal.others.toString(), totalFee: nagal.totalFee.toString(), received: nagal.received.toString(), balance: nagal.balance.toString() });
    setShowModal(true);
  };
  ...
  const handleSave = async () => {
    ...
    try {
        if (isEdit) { 
           await fetch(`/api/documents/${currentNagal.id}`, {
              method: 'PUT',
